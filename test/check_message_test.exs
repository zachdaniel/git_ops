defmodule GitOps.Mix.Tasks.Test.CheckMessageTest do
  ...
  describe "with valid path" do
    setup do
      ...
      on_exit(fn -> delete_temp_msg_file!(message_file_name) end)
    ...
    test "check incorrect message", %{message_file_name: message_file_name} do
      temp_file_name =
        create_temp_msg_file!(message_file_name, """
  ...
  defp delete_temp_msg_file!(name) do
    tmp_file = Path.join(System.tmp_dir!(), name)
    File.rm!(tmp_file)
  end

  defp create_temp_msg_file!(name, content) do
    tmp_file = Path.join(System.tmp_dir!(), name)
